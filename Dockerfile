...
# Set permissions to 777 for cache directory and parent to ensure nginx worker can write
# Note: Cache directory needs write access for on-the-fly conversion
RUN mkdir -p /usr/local/openresty/nginx/html/assets/.webp_cache && \
    chmod 777 /usr/local/openresty/nginx/html/assets && \
    chmod 777 /usr/local/openresty/nginx/html/assets/.webp_cache && \
